...
function emptyRow() {
  return {
    ...
    note: "",
    // result: ""  // ❌ bỏ UI chỉnh sửa/hiển thị trong bảng
  };
}
  ...
  return (
    <style>{`
      @media print {
        ...
        #__print_root__ { display: block !important; position: static !important; inset: auto !important;
          background: white !important; padding: 24px !important; z-index: 999999 !important; box-shadow: none !important; }
        table, th, td { border-color: #000 !important; }
      }
    `}</style>
  );
}
function PrintSheet({ show, patient, services = [] }) {
  if (!show) return null;
  const node = (
    <div id="__print_root__" className="__print-overlay__ fixed inset-0 z-[9999] bg-white p-6 overflow-auto">
      <PrintStyles />
      <h1 className="text-center text-teal-700 font-extrabold text-xl mb-3">PHIẾU CHỈ ĐỊNH DỊCH VỤ</h1>

      {/* Thông tin BN */}
      <table className="w-full border-collapse mb-4 text-sm">
        <tbody>
          <tr>
            <td className="border border-slate-300 p-2 font-semibold">Họ tên</td>
            <td className="border border-slate-300 p-2">{patient?.name}</td>
            <td className="border border-slate-300 p-2 font-semibold">Mã BN</td>
            <td className="border border-slate-300 p-2">{patient?.pid || patient?.id}</td>
          </tr>
          <tr>
            <td className="border border-slate-300 p-2 font-semibold">Tuổi</td>
            <td className="border border-slate-300 p-2">{patient?.age ?? "-"}</td>
            <td className="border border-slate-300 p-2 font-semibold">Giới tính</td>
            <td className="border border-slate-300 p-2">{patient?.gender ?? "-"}</td>
          </tr>
          <tr>
            <td className="border border-slate-300 p-2 font-semibold">Khoa</td>
            <td className="border border-slate-300 p-2" colSpan={3}>{patient?.dept || patient?.department || "-"}</td>
          </tr>
          <tr>
            <td className="border border-slate-300 p-2 font-semibold">Ngày giờ</td>
      ...
      <h3 className="mt-3 mb-1 font-bold">Dịch vụ được chỉ định</h3>
      {services.length ? (
        <ul className="list-disc pl-5">
          {services.map((n, i) => <li key={`${n}-${i}`}>{n}</li>)}
        </ul>
      ) : (
        <div className="border border-slate-300 p-2">- Không có dịch vụ -</div>
      )}
    </div>
  );
...
export default function ExamDetail({ patient, onBack, onExportDiagnosis, onExportOrder }) {
  ...
  const hasDx = useMemo(() => !!(dx.pre || dx.final || dx.plan || dx.advice), [dx]);
  ...
  // Prefill nếu flow khám dịch vụ
  useEffect(() => {
    const items = patient?.serviceOrder?.items;
    if (Array.isArray(items) && items.length) {
      setRows(items.map(sid => ({ ...emptyRow(), svcId: (EXAM_SERVICES.find(s => s.id === sid)?.id) || sid })));
    }
  ...
  // Build payload (vẫn giữ r.result trong payload nếu sau này service trả về)
    ...
    const orderRows = rows
      ...
      .map(r => {
        ...
        return {
          ...
          note: r.note || "",
          result: r.result || "", // có thể rỗng vì không nhập trong UI
        };
      });
  ...
  const printStartedAtRef = React.useRef(0);
  useEffect(() => {
    function onAfterPrint() {
      if (Date.now() - printStartedAtRef.current < 600) return;
      setPrinting(false);
    }
  ...
  function handleExportOrderAndPrint() {
    if (!hasOrder) return;
    const payload = buildPayloadCommon();
    onExportOrder?.(patient, payload);

    const services = payload.orderRows.map(r => r.serviceName);
    ...
    setTimeout(() => setPrinting(false), 20000);
  }

  /* ======== TONE: TEAL (khác toàn bộ trang khác) ======== */
  ...
  // ======= Kết quả dịch vụ (chỉ xem) — lấy từ patient.pendingServiceResults hoặc patient.serviceResults =======
    ...
    <>
      <section className="h-full min-h-0 flex flex-col">
        ...
        <motion.div {...fadeIn} className="flex-1 min-h-0 overflow-y-auto scrollbar-none px-1 pt-3 pb-0">
          ...
          {/* PHIẾU KHÁM — bỏ cột KẾT QUẢ trong bảng */}
              ...
              <div className="flex gap-2">
                <motion.div whileTap={{ scale: 0.97 }}>
                  <Button
                    variant="radigan"
                    disabled={!hasOrder}
                    onClick={handleExportOrderAndPrint}
                    title={!hasOrder ? "Chọn ít nhất 1 dịch vụ" : "Xuất phiếu khám & in phiếu"}
                  >
                    Xuất phiếu khám
                  </Button>
                </motion.div>
          ...
          <motion.section {...fadeIn} className="rounded-2xl p-4 mt-3 bg-white shadow-sm border border-slate-200">
            ...
            <div className="mt-2 rounded-xl bg-white shadow-inner/10 max-h-56 overflow-y-auto scrollbar-none border border-slate-200">
              <table className="min-w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {rx.map((r, i) => (
                    <tr key={r.code || `${r.name}-${i}`} className={`transition-colors ${toneHover}`}>
          ...
          <div className="px-4 pt-3 flex justify-end">
            <motion.div whileTap={{ scale: 0.97 }}>
              <Button
                variant="radigan"
                disabled={!(hasOrder && hasDx)}
                aria-disabled={!(hasOrder && hasDx)}
                onClick={handleExportDiagnosis}
                title={!(hasOrder && hasDx) ? "Cần có chỉ định & chẩn đoán" : "Xuất phiếu chẩn đoán & kết thúc khám"}
              >
      ...
      {/* PRINT OVERLAY */}
      <PrintSheet show={printing} patient={patient} services={printServices} />
